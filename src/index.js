...
*/	
	...
	current_id = $.trim($('#queryText').val())
      ...
      circles = circles.enter().append('circle')
        .attr('r', circleSize)
        .attr('fill', function(d){var col =""; if (d.properties.PMCID == current_id || d.properties.PMID == current_id){col = "#1E90FF"} else {col = getItemColor(d)} return col} )
        .attr('stroke',"black")
        .call(drag_handler)
        .on('mouseover', function(d) {
          d3.select(this)
            .attr('fill', function(d) {var col =""; if (d.properties.PMCID == current_id || d.properties.PMID == current_id){col = "#AFEEEE"} else {col = getColorBrighter(getItemColor(d))} return col})
            .attr('stroke', "gray");
          showProperties(d);
        })
      ...
      circleText = circleText.enter().append('text')
        ...
        .text(function(d){if (d.labels[0] == "Paper" ){ var t = d.properties.Title} else { var t = d.properties.Term};return t.substr(0,6);})
        .merge(circleText);
          ...
          return;
			...
			if ($('#chkboxCypherQry:checked').val() != 1){
				if (queryStr.substr(0,3) == "PMC"){
						queryStr = 'OPTIONAL MATCH zz=(p:Paper{PMCID:\''+ queryStr + '\'})-[:ACCESSION]->(acs:Accession) ' +
						'OPTIONAL MATCH pp= (p:Paper{PMCID:\''+ queryStr + '\'})-[:ACCESSION]->(acs:Accession)<-[:ACCESSION]-(:Paper) RETURN pp, zz'
						}
				else {queryStr = 'OPTIONAL MATCH zz=(p:Paper{PMID:\''+ queryStr + '\'})-[:ACCESSION]->(acs:Accession) ' +
						'OPTIONAL MATCH pp= (p:Paper{PMID:\''+ queryStr + '\'})-[:ACCESSION]->(acs:Accession)<-[:ACCESSION]-(:Paper) RETURN pp, zz'};
    ...
    $(function() {
      ...
      $('#chkboxCypherQry').change(function() {
        if (this.checked)
          ...
          $('#queryText').prop('placeholder', 'PMCID or PMID');
      });
    });
